...
const enforcements = [
    ...
    },
    {
        ...
        amount: 0,
        date: "2024-02-20",
        summary: "Finanstilsynet ga Nordisk Forsikring AS en offentlig advarsel etter stedlig tilsyn avdekket alvorlige mangler i foretakets IKT-sikkerhet. Tilsynet konstaterte mangelfull tilgangskontroll til kritiske systemer, manglende gjennomforing av penetrasjonstester, og utilstrekkelig hendelseshaandtering. Foretaket fikk frist pa seks maneder til a bringe IKT-sikkerheten i samsvar med IKT-forskriften og rapportere gjennomforte tiltak til Finanstilsynet.",
...
// -- Summary --
const provisionCount = db.prepare("SELECT count(*) as cnt FROM provisions").get().cnt;
const sourcebookCount = db.prepare("SELECT count(*) as cnt FROM sourcebooks").get().cnt;
const enforcementCount = db.prepare("SELECT count(*) as cnt FROM enforcement_actions").get().cnt;
const ftsCount = db.prepare("SELECT count(*) as cnt FROM provisions_fts").get().cnt;
console.log(`\nDatabase summary:`);
